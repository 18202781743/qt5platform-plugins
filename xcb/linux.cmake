...
if (NOT xcb-xlib)
    ...
    if (${Qt5_VERSION_MINOR} GREATER 14)
        ...
        if (${Qt5_VERSION_MINOR} GREATER 14)
            if (xcb-xinput)
                add_definitions(-DXCB_USE_XINPUT2 XCB_USE_XINPUT21 XCB_USE_XINPUT22)
                list(APPEND XCB_USE xcb_xinput)
                if (QMAKE_LIBXI_VERSION_MAJOR)
                    add_definitions(-DLIBXI_MAJOR=${QMAKE_LIBXI_VERSION_MAJOR})
                    add_definitions(-DLIBXI_MINOR=${QMAKE_LIBXI_VERSION_MINOR})
                    add_definitions(-DLIBXI_PATCH=${QMAKE_LIBXI_VERSION_PATCH})
                endif()
            endif()
        else()
            if (xinput2)
                add_definitions(-DXCB_USE_XINPUT2)
                list(APPEND XCB_USE xinput2)
                if (QMAKE_LIBXI_VERSION_MAJOR)
                    add_definitions(-DLIBXI_MAJOR=${QMAKE_LIBXI_VERSION_MAJOR})
                    add_definitions(-DLIBXI_MINOR=${QMAKE_LIBXI_VERSION_MINOR})
                    add_definitions(-DLIBXI_PATCH=${QMAKE_LIBXI_VERSION_PATCH})
...
if(xcb-sm)
    ...
    list(APPEND XCB_USE x11sm)
endif()

if(NOT system-xcb)
    add_definitions(-DXCB_USE_RENDER)
    list(APPEND XCB_USE xcb)
else()
    if(xkb)
        list(APPEND XCB_USE xcb_xkb)
    endif()
    # to support custom cursors with depth > 1
    if (xcb-render)
        add_definitions(-DXCB_USE_RENDER)
        list(APPEND XCB_USE xcb_render)
    endif()
    list(APPEND XCB_USE xcb_syslibs)
endif()
